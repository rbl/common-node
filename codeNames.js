...
function CodeNames(options) {
    var self = this;
    Events.EventEmitter.call(this);
    
    this.options = PK.deepCopy(options) || {};
    this.words = {};
    this.strBuffer = "";
    
    this.options.datafile = this.options.datafile || __dirname+"/codeNamesData.txt";
    if (this.options.loadSync) {
        Logger.debug("Synchroous load of code name data")
        var data = FS.readFileSync(this.options.datafile, "utf-8");
        ...
        Logger.debug("Starting read of code name word list");
        this.reader = FS.createReadStream(this.options.datafile,{encoding:"utf-8"});
    
        this.reader.on("data", function onData(data) {
            self.loadOnData(data);
        });
...
module.exports.createSharedSync = function() {
    if (shared) return;
    
    shared = new CodeNames({loadSync:true});
}
...
CodeNames.prototype.loadOnData = function(data) {
    if (!data) return;
    ...
    var ix;
    var last = 0;
    while ((ix = this.strBuffer.indexOf('\n', last)) != -1) {
        this.addWord(this.strBuffer.slice(last, ix));
        last = ix + 1;        
    }
    if (last > 0) this.strBuffer = this.strBuffer.slice(last);
}

CodeNames.prototype.loadOnEnd = function() {
    var self = this;
...
// Test
if (!module.parent) {
    var names = new CodeNames({loadSync:true});
